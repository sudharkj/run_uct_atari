...
parser.add_argument('--rollout_agent_name', type=str, default=None, help="Rollout agent name: [random, cnn, rnn]")
...
def run(env, version, act_repeats, max_steps, rollout_agent_name, behavior_agent_name, eps_greedy,
        ...
        save_dir, save_frequency, process):
...
def collect_data(env, version, act_repeats, max_steps, rollout_agent_name, behavior_agent_name, eps_greedy,
                 num_simulations, search_horizontal, gamma, exploration, prune_tree, report_frequency, runs_per_worker,
                 num_workers, save_dir, save_frequency):
    ...
    for i in range(num_workers):
        worker = Process(target=run, args=(
            ...
            search_horizontal, gamma, exploration, prune_tree, report_frequency, runs_per_worker, save_dir, save_frequency, i
        ))
        worker.daemon = True
        worker.start()
...
if __name__ == '__main__':
    ...
    collect_data(
        ...
        args.behavior_agent_name, args.eps_greedy, args.num_simulations, args.search_horizontal, args.gamma, args.exploration,
        args.prune_tree, args.report_frequency, args.runs_per_worker, args.num_workers, args.save_dir,
        args.save_frequency
    )
